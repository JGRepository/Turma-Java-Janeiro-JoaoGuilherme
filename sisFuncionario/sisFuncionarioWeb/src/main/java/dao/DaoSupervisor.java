...
public class DaoSupervisor {
	...
	public static boolean salvarSupervisor(SupervisorAuxiliar supervisorAuxiliar, String query) {
		...
		try {
			...
			preparaOcomandoSQL = connection.prepareStatement(sqlQuery);

			if ("INSERT".equalsIgnoreCase(query)) {
				preparaOcomandoSQL.setString(1, supervisorAuxiliar.getCpf());
				preparaOcomandoSQL.setString(2, supervisorAuxiliar.getNome());
				preparaOcomandoSQL.setString(3, supervisorAuxiliar.getEmail());
			} else if ("UPDATE".equalsIgnoreCase(query)) {
				preparaOcomandoSQL.setString(1, supervisorAuxiliar.getNome());
				preparaOcomandoSQL.setString(2, supervisorAuxiliar.getEmail());
				preparaOcomandoSQL.setString(3, supervisorAuxiliar.getCpf());
			} else if ("DELETE".equalsIgnoreCase(query)) {
				preparaOcomandoSQL.setString(1, supervisorAuxiliar.getCpf());
