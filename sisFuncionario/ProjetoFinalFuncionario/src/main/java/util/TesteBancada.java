...
//		System.out.println("INSERT interno: " + okInsert);
		...
		// 2) LIST
		List<FuncionarioTerceirizado> lista = dao.DaoFuncTerceirizado.listarFuncTerceirizadoNoBanco();
		System.out.println("TOTAL internos: " + lista.size());
		for (FuncionarioTerceirizado item : lista) {
			System.out.println(item.getCpf() + " | " + item.getNome() + " | " + item.getEmpresa() + " | PLR="
					+ item.getHorasTrabalhadas() + " | " + item.getSenha());
		}
